module ApplicationHelper
  ...
  def word_chart(user, words, good)
    ...
    unless partitioned_words[0].empty?
      ...
      chart << "<img src=\"#{img_src.join('')}\" width=\"600\" height=\"473\" alt=\"Words that #{good ? "best" : "don't"} describe #{name}\"/>"
    end

    unless partitioned_words[1].empty?
      chart << "<p>"
      chart << "#{name} is #{"also " unless partitioned_words[1].empty?}#{"not" unless good} (each of these words has only been suggested once):"
      chart << "</p>"
